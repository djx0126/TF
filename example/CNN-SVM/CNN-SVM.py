...
right1 = 0  # 记录预测为0且实际为0的结果数
error1 = 0.0  # 记录预测为0但实际为1的结果数


def transform_str_array_as_list(strs):
    float_list = list(map(float, strs))
...
def read_file_data(file_name):
    ...
    for i in range(len(rr)):
        x_list1 = transform_str_array_as_list(rr[i].split(' ')[:256])
        x.append(x_list1)
        y_list1 = transform_str_array_as_list(rr[i].split(' ')[256:266])
        y.append(y_list1)
    ff.close()
    return x, y
...
for file_num in range(10):
    ...
    x_test2, y_test2 = read_file_data('digit_train_' + file_num.__str__() + '.data')

    # 以上是读出训练数据
    x_test3, y_test3 = read_file_data('digit_test_' + file_num.__str__() + '.data')
    # 以上是读出测试数据
    ...
    for j in range(len(x_temp2)):
        # 验证时出现四种情况分别对应四个变量存储
        x_temp2_reshape = np.array(x_temp2[j]).reshape((1, -1)) # reshape required by sklearn
        predict_value = clf.predict(x_temp2_reshape)[0]

        if predict_value == y_test3[j] == 1:
            right0 += 1
            right0_svm += 1
        elif predict_value == y_test3[j] == 0:
            right1 += 1
            right1_svm += 1
        elif predict_value == 1 and y_test3[j] == 0:
            error0 += 1
            error0_svm += 1
        else:
            error1 += 1
    ...
    accuracy_svm = (right0_svm + right1_svm) / (right0_svm + error0_svm + right1_svm + error1_svm)
    print('svm step ratio %g' % accuracy_svm)

accuracy = (right0) / (right0 + error0)  # 准确率
recall = (right0) / (right0 + error1)  # 召回率
print('svm right ratio %g' % ((right0 + right1) / (right0 + right1 + error0 + error1)))
print('accuracy %g' % accuracy)
print('recall %g' % recall)
print('F1 score %g' % (2 * accuracy * recall / (accuracy + recall)))  # 计算F1值
